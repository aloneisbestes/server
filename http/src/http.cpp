...
HttpConn::HttpCode HttpConn::processData() {
    ...
    if (m_method == Method::GET) {
        source_path = m_root + m_url;
        DebugPrint("source_path: %s\n", source_path.c_str());
        if (strncasecmp(m_url.c_str(), "/images", 7) == 0) {
            m_out_file.open(source_path, std::fstream::in | std::fstream::binary);
        } else {
            m_out_file.open(source_path, std::fstream::in);
        }
...
void HttpConn::writeData(HttpConn::HttpCode code) {
    ...
    if (strncasecmp(m_url.c_str(), "/images", 7) == 0) {
        ...
        while (!m_out_file.eof()) {
            m_out_file.getline(tmpbuf, 1024);
    ...
    if (flag == true) {
        ...
        n = send(m_sockfd, m_resp_content.c_str(), m_resp_content.size(), 0);
    }
